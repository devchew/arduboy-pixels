...
export function exportSprite(sprite: SpriteData, options: ExportOptions): string {
  const { format, includeDimensions, includeFrameCount, variableName, compress } = options;
  ...
  const byteArray = sprite.isAnimation && sprite.frames 
    ? convertAnimationToBytes(sprite) 
    : convertSpriteToBytes(sprite.pixels);
  
  // Generate array declaration
  const arrayType = format === 'progmem' ? 'const unsigned char' : 'const uint8_t';
  ...
  for (let i = 0; i < byteArray.length; i += bytesPerLine) {
    ...
    const formattedBytes = line.map(byte => `0x${byte.toString(16).padStart(2, '0').toUpperCase()}`);
...
export function exportProject(sprites: SpriteData[], projectName: string): string {
  let output = '';

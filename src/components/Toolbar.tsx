...
const tools: { id: DrawingTool; icon: React.ReactNode; label: string; shortcut?: string }[] = [
  ...
  { id: 'filled-rectangle', icon: <Square size={18} />, label: 'Filled Rectangle' },
  { id: 'circle', icon: <Circle size={18} />, label: 'Circle', shortcut: 'C' },
  { id: 'filled-circle', icon: <Circle size={18} />, label: 'Filled Circle' },
  { id: 'invert', icon: <Invert size={18} />, label: 'Invert Selection', shortcut: 'I' }
];
        ...
        <div className="flex items-center space-x-1">
          <button
            ...
            className={`p-2 rounded ${
              canUndo ? 'hover:bg-gray-700 text-gray-300' : 'text-gray-600 cursor-not-allowed'
            }`}
            title="Undo (Ctrl+Z)"
          >
            <RotateCcw size={18} />
          ...
          <button
            ...
            disabled={!canRedo}
            className={`p-2 rounded ${
              canRedo ? 'hover:bg-gray-700 text-gray-300' : 'text-gray-600 cursor-not-allowed'
            }`}
            title="Redo (Ctrl+Y)"
          >
            <RotateCw size={18} />
